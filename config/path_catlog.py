...
class DatasetCatalog:
    ...
    def get(name):
        ...
        #添加自己的数据集
        elif "welllid" in name:
            welllid_root = DatasetCatalog.DATA_DIR
            if 'welllid_ROOT' in os.environ:
                welllid_root = os.environ['welllid_ROOT']

            attrs = DatasetCatalog.DATASETS[name]
            args = dict(
                data_dir=os.path.join(welllid_root, attrs["data_dir"]),
                split=attrs["split"],
            )
            return dict(factory="welllidDataset", args=args)
